...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/09/06 17:30:07 by anoury            #+#    #+#             */
/*   Updated: 2022/09/06 17:30:08 by anoury           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*fil_all(char *buf)
{
	...
	if (!buf[i])
		return (NULL);
	str = (char *)malloc(sizeof(char) * (ft_strlen2(buf) + 1));
	if (!str)
		return (NULL);
	while (buf[i])
	{
...
char	*buff_rest(char	*buf)
{
	...
	while (buf[i] != '\n')
		i++;
	ptr = malloc(sizeof(char) * (ft_strlen2(buf) - i));
	if (!ptr)
		return (NULL);
	i++;
	j = 0;
